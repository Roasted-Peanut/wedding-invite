...
const FallingItems = React.memo(() => {
  ...
  const resizeCanvas = useCallback((canvas, ctx, dpr) => {
  ...
  const initItems = useCallback((images, width, height) => {
  ...
  const onResizeReinit = useCallback((canvas, ctx, dpr, images, setItems) => {
    const { width, height } = resizeCanvas(canvas, ctx, dpr);
    const newItems = initItems(images, width, height);
    setItems(newItems);
  }, [resizeCanvas, initItems]);
  ...
  useEffect(() => {
    ...
    // Initial setup
    const { width: canvasWidth, height: canvasHeight } = resizeCanvas(canvas, ctx, dpr);
    width = canvasWidth;
    height = canvasHeight;
    ...
    const handleResize = () => {
      const { width: newWidth, height: newHeight } = resizeCanvas(canvas, ctx, dpr);
      width = newWidth;
      height = newHeight;
      items = initItems(images, width, height);
    };
  ...
  }, [loadImages, resizeCanvas, initItems]);
...
FallingItems.displayName = "FallingItems";
